...
class VideoDataProcessor:
    ...
    def create_indexes(self):
        try:
            ...
            # 为embedding字段创建向量索引
            # 注意：这里的维度1024需要根据实际情况修改，要和数据库中保持一致
            if "vector_index" not in existing_indexes:
                collection.create_index([("embedding", "vector")], name="vector_index",
                                        vectorOptions={
                                            "type": "hnsw",
                                            "dimensions": 1024,
            ...
            # 为时间戳字段创建索引（可选，根据实际查询需求决定是否创建）
            timestamp_index_name = "start_timestamp_millis_1_end_timestamp_millis_1"
            if timestamp_index_name not in existing_indexes:
            ...
            print(f"Error creating indexes in DocumentDB: {str(e)}")
    ...
    def store_in_documentdb(self, flattened_data):
        ...
        try:
            ...
            if flattened_data:
                ...
                print(f"Successfully stored {len(result.inserted_ids)} documents in DocumentDB")
            else:
    ...
    def process_video_data(self, event):
        try:
            ...
            # 从S3 key提取视频名称
            # 例如从 "video-output/example.mp4/result.json" 提取 "example.mp4"
            parts = key.split('/')
            ...
            for chapter in video_data.get('chapters', []):
                ...
                chapter_data = {
                    ...
                    'chapter_summary': {
                        ...
                        'embedding': self.get_embeddings(chapter_summary)
                    },
            ...
            doc_data = {
                ...
                'video_summary': {
                    'text': video_summary,
                    'embedding': self.get_embeddings(video_summary)
                },
